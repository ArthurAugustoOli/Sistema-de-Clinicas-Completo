<?php
/* Nome: notificacoes.php | Caminho: /includes/notificacoes.php */
...
function getNotificacoes()
{
    ...
    $sql = "SELECT n.*, u.nome as criador_nome 
            FROM notificacoes n 
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $sql_lida = "SELECT lida FROM notificacoes_usuarios WHERE notificacao_id = ? AND usuario_id = ?";
                ...
                if ($stmt_lida) {
                    ...
                    if ($result_lida->num_rows > 0) {
                        ...
                        $sql_insert = "INSERT INTO notificacoes_usuarios (notificacao_id, usuario_id, lida) VALUES (?, ?, 0)";
...
function contarNotificacoesNaoLidas()
{
    ...
    $usuario_id = $_SESSION['id'];
    $count = 0;
    $sql = "SELECT COUNT(*) as count FROM notificacoes n 
            LEFT JOIN notificacoes_usuarios nu ON n.id = nu.notificacao_id AND nu.usuario_id = ?
            WHERE (n.para_todos = 1 OR nu.usuario_id = ?)
            AND (nu.lida = 0 OR nu.lida IS NULL)
            AND (n.expira_em IS NULL OR n.expira_em > NOW())";
    if ($stmt = $conn->prepare($sql)) {
...
$notificacoes_nao_lidas = contarNotificacoesNaoLidas();
?>
...
<div class="dropdown-menu notifications-dropdown" id="notificacoesMenu">
    ...
    <div class="dropdown-body" id="notificacoesList">
        <?php $notificacoes = getNotificacoes(); ?>
                ...
                <div class="notification-item <?= $notificacao['lida'] ? '' : 'unread' ?>" data-id="<?= $notificacao['id'] ?>">
                    ...
                    <div class="notification-content">
                        ...
                        <div class="notification-meta">
                            <span class="time"><?= date('d/m/Y H:i', strtotime($notificacao['data_criacao'])) ?></span>
                            <span class="author">Por: <?= htmlspecialchars($notificacao['criador_nome']) ?></span>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <div class="dropdown-footer">
        <a href="<?= $baseUrl ?>pages/todas_notificacoes.php">Ver todas as notificações</a>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (enviarNotificacaoBtn) {
            enviarNotificacaoBtn.addEventListener('click', function() {
                ...
                fetch('<?= $baseUrl ?>api/notificacoes/criar.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
